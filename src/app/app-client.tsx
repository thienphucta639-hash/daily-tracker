...
function InvModal({ date, exps, onClose }: { date: string; exps: S.Expense[]; onClose: () => void }) {
  ...
  return (<Wrap title="Hóa đơn" onClose={onClose}>
    ...
    <div className="flex gap-2 mt-3"><button onClick={() => { const d = encodeURIComponent(JSON.stringify({ date, total, items: exps.map(e => ({ description: e.description, amount: e.amount, category: e.category })) })); window.open(`https://saving-y2k.vercel.app/?expenses=${d}`, "_blank"); }} className="flex-1 flex items-center justify-center gap-1.5 py-2 bg-gold/15 hover:bg-gold/25 text-gold border border-gold/30 rounded-lg text-xs font-bold transition-colors active:scale-[0.98]"><Ic d={P.wallet} size={13} /> Tiết kiệm</button></div>
  </Wrap>);
}
